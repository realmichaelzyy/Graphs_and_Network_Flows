...
h=open("individual.txt",'r')
w=open("rating_num.txt",'r')
q=open("prob8_individual.txt",'w')
...
for i in xrange(len(tag_dir)):
	# line="%.1f %d %d\n"%(tag_rating[i],tag_actRank1[i],tag_dir[i])
	line="%.10f %.10f %.10f %.10f %.10f %d\n"%(tag_actRank1[i],tag_actRank2[i],tag_actRank3[i],tag_actRank4[i],tag_actRank5[i],tag_dir[i])
	q.write(line)
